...
require_once ASP_PLUGIN_DIR_PATH . '/admin/WL_ASP_LC.php';

final class WL_ASP_LM {
    ...
    private function __construct() {
        $this->license_key = trim( get_option( 'wl-asp-key' ) );
    }
    ...
    public function is_valid() {
        return true;
        if( get_option( 'wl-asp-valid' ) ) {
            return true;
        }
        if( $this->key_exists() ) {
            $checker = new WL_ASP_LC( $this->license_key, $this->api_url, md5( $this->license_key ) );
            if ( ! $checker->checkLicenseKey() ) {
                $this->error_message = $checker->getMessage();
                return false;
            } else {
                $this->error_message = null;
                $activation_cache = trim( get_option( 'wl-asp-cache' ) );
                ...
                update_option( 'wl-asp-key', $this->license_key );
                update_option( 'wl-asp-valid', true );
                if ( $prev_activation_cache != $activation_cache ) {
                    update_option( 'wl-asp-cache', $activation_cache );
                }
                return true;
            }
        }

...
jQuery(document).ready(function() {
	var table = jQuery('#display_holiday').DataTable( {
		...
		"language": {
			"loadingRecords": "<?php _e('No Holiday Add',WL_A_P_SYSTEM); ?>"
		},
	} );
} );
...
function  holiday_save()
{
	...
	jQuery('.save_appt_holiday').html('<i class="fas fa-spinner fa-spin"></i> <?php _e("Creating",WL_A_P_SYSTEM); ?>');  
	...
	jQuery.ajax({
	  ...
	  function  holiday_update()
	  {
	  	...
	  	jQuery('.update_appt_holiday').html('<i class="fas fa-spinner fa-spin"></i> <?php _e("Updating",WL_A_P_SYSTEM); ?>');
	  	jQuery.ajax({
	  		url: location.href,
	  		type: "POST",
		...
		success: function (html) {
			...
			jQuery('.update_appt_holiday').html('<?php _e("Update",WL_A_P_SYSTEM); ?>');
...
jQuery(function(){
	jQuery("a.delete_holiday").click(function(){
		...
		jQuery("#my_check_holiday:checked").each(function(i){
		 ...
		 if(ids.length == 0){
 			jQuery.notify("<?php _e('Please Select Atleast One Holiday To Delete',WL_A_P_SYSTEM); ?>", {type:"success",icon:"check",align:"center", verticalAlign:"middle",color: "#3c763d", background: "#dff0d8"});
 		}
 		else{
		jQuery.confirm({
			title: '<?php _e("Please Confirm",WL_A_P_SYSTEM); ?>',
			...
			confirm: function () {
				...
				jQuery('.delete_holiday').html('<i class="fas fa-spinner fa-spin"></i> <?php _e("Deleting",WL_A_P_SYSTEM); ?>');			 
...
// multi delete
 ...
 <div class="panel panel-default">
 	<div class="panel-heading"><i class="fas fa-coffee"></i><span class="panel_heading"><?php _e('Holiday',WL_A_P_SYSTEM); ?></span>
 		<div class="theme-export">
 			<button type="button" class="btn theme-customer add-customer" data-toggle="modal" data-target="#holi_day"> <i class="fas fa-plus" aria-hidden="true"></i><?php _e(' Add Holiday',WL_A_P_SYSTEM); ?></button>
 		</div>
 	</div>
 	<div class="panel-body">
 								...
 								<form action="" method="post" id="form_holiday">									
 									...
 									<div class="row">
										<div class="col-md-6">
	 									...
	 									<div class="ad-ser hide_repeat_day all_hide" style="display:none">
	 										<div class="col-md-12 col-sm-12">
	 									...
	 									<div class="ad-ser hide_repeat_week all_hide" style="display:none">	  
	 										<div class="col-md-12 col-sm-12">
	 									...
	 									<div class="ad-ser hide_bi_repeat_week all_hide" style="display:none">
	 										<div class="col-md-12 col-sm-12">
	 									...
	 									<div class="ad-ser hide_repeat_month all_hide" style="display:none">
	 										<div class="col-md-12 col-sm-12">
	 											<label ><?php _e('Repeat Month(s)',WL_A_P_SYSTEM); ?></label>	
	 											<input name="re_months" id="re_months" type="number" value="0"  maxlength="2" class="form-control" value="<?php echo date("m/d/Y"); ?>" />	
	 										</div>
	 									</div>
 									</div>
 				...
 				<div class="modal-dialog">
 					...
 					<div class="modal-content">
 						...
 						<div class="col-md-12 modal-body">
 							<div class="form-group" id="holiday_html_fetch">
 		...
 		<div id="mydiv" class="table-responsive">
 			<form method="post" id="holiday_del" name="holiday_del" >
 				...
 				<a href="#" class="delete_holiday btn btn-link"> <i class="fas fa-trash-o" aria-hidden="true"></i><?php _e(' Delete',WL_A_P_SYSTEM); ?></a></td>
 			</form>
 		</div>
 	</div>
 </div>

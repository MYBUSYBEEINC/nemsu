...
if ( !class_exists('ASP_UpdateChecker') ):
...
class ASP_UpdateChecker {
	 ...
	 * @return void
	 */
	public function ASP_installHooks(){		
		//Add custom Plugin update menu at Dashboard update menu section.
		add_action('admin_menu', array( $this, 'ASP_my_Plugin_menu'));	
		//Add custom Plugin update available notice with link menu at top menu section.
		add_action('admin_bar_menu', array( $this, 'ASP_Plugin_update_link'), 99999999);
		//Insert our update css style by this function at footer.
		add_action('admin_footer', array( $this, 'ASP_my_admin_Plugin_update_function'));		
	}

	public function ASP_getNewVersion() {
		$details_url = 'https://weblizar.com/plugins/appointment-scheduler-pro/';
		$download_url = 'https://weblizar.com/amember/login';
		$updation_detail = unserialize(get_option('Appointment_scheduler_updation_detail'));
		...
		if( isset( $updation_detail['next_check'] ) && ( strtotime(date('Y-m-d')) >= strtotime($updation_detail['next_check']) ) ) {
			$update_version = $this->ASP_requestUpdate();
			if($update_version) {
				$new_version->version = $update_version->version;
			} else {
				$new_version->version = $this->ASP_getInstalledVersion();
			}
	...
	public function ASP_Plugin_update_link($wp_admin_bar) 
	{
		$new_version = $this->ASP_getNewVersion();		
		$old_version = $this->ASP_getInstalledVersion();
		if ( is_object($new_version) && version_compare($new_version->version, $old_version, '>') )
		{
		$args = array(
				'id' => 'ASP_Plugin_update_menu',
				'title' => 'Appointment Scheduler Pro Update <span style="color:#fff;font-weight:bold;">Update Available !</span>', 
				'href' => get_admin_url().'index.php?page=ASP_new_updates',  
				'meta' => array(
					'class' => 'ASP_update-link',
					'title' => 'ASP_Update Available'
					)
			);
	...
	public function ASP_my_Plugin_menu() {
		$new_version = $this->ASP_getNewVersion();		
		$old_version = $this->ASP_getInstalledVersion();
		if ( is_object($new_version) && version_compare($new_version->version, $old_version, '>') )
		{
			add_dashboard_page('Appointment Scheduler Pro Update', 'Appointment Scheduler Pro Update', 'read', 'ASP_new_updates', array( $this, 'ASP_my_Plugin_function'));
		}
	}
	public function ASP_my_Plugin_function() { 
		$new_version = $this->ASP_getNewVersion();		
		$old_version = $this->ASP_getInstalledVersion();
		if ( is_object($new_version) && version_compare($new_version->version, $old_version, '>') )
		{
		...
		$msg = sprintf( $msg, $this->product_name );
			...
			<div class="weblizar_new_updates">
				...
				<a target="_blank" href="<?php echo ASP_PLUGIN_URL; ?>" title="Installing Weblizar WordPress Plugin"><?php _e($this->product_name);?></a>
				</h1>
				<div class="weblizar_new_updates_details">
					...
					<h2 class="weblizar_new_updates_details_main_headings" ><?php _e('Weblizar Plugin Update Process','weblizar');?></h2>			
						...
						<p><img style="margin: 20px 0; width: 750px;" src="<?php echo plugin_dir_url( __FILE__ ).'images/appointment-pro.jpg'; ?>" alt="wp-dashboard"></p>
						...
						<p><img style="margin: 20px 0; width: 750px;" src="<?php echo plugin_dir_url( __FILE__ ).'images/install-appointment.jpg'; ?>" alt="wordpress-Plugin-install"></p>
						<p><?php _e('Your Weblizar Pro Plugin Activated Now','weblizar');?></p>
						<p><?php _e('For more details to follow the Plugin documentation link ','weblizar');?><a target="_blank" href='https://weblizar.com/documentation/plugins/appointment-scheduler-pro/'><?php _e("Appointment Scheduler Pro",'weblizar'); ?></a></p>
	...
	<?php
		...
		function ASP_my_admin_Plugin_update_function() { ?>	
			<style>
				...
				#wp-admin-bar-ASP_Plugin_update_menu.ASP_update-link{background-color:#31A3DD !Important;border-radius:5px!important;}
				#wp-admin-bar-ASP_Plugin_update_menu.ASP_update-link a{color:#fff !Important;}
				#wp-admin-bar-ASP_Plugin_update_menu{margin-right:5px !Important;}				
 ...
 /**
	 ...
	 * @return PluginUpdate 
	 */
	public function ASP_requestUpdate($queryArgs = array()){
		//Query args to append to the URL. Plugins can add their own by using a filter callback (see addQueryArgFilter()).
		$queryArgs['installed_version'] = $this->ASP_getInstalledVersion(); 
		...
		if ( ($code == 200) && !empty($body) ){
			...
			if ($PluginUpdate != null) {
				$updationDetail = serialize(array(
							'current_version' => $this->ASP_getInstalledVersion(),
							'new_version' => $PluginUpdate->version,
							'checked_on' => date('Y-m-d'),
							'check_again' => version_compare($PluginUpdate->version, $this->ASP_getInstalledVersion(), '>') ? true : false, 
							'next_check' => date('Y-m-d', strtotime("+1 day"))
						));
				update_option('Appointment_scheduler_updation_detail', $updationDetail);
			}

			//The update should be newer than the currently installed version.
			if ( ($PluginUpdate != null) && version_compare($PluginUpdate->version, $this->ASP_getInstalledVersion(), '<') ){
				$PluginUpdate = null;
			}
		}
	 ...
	 */
	public function ASP_getInstalledVersion(){
		return ASP_VERSION;
	}
}
...
$weblizar_update_checker = new ASP_UpdateChecker('Appointment Scheduler Pro','https://weblizar.com/updates/plugins/appointment-scheduler-pro.json');
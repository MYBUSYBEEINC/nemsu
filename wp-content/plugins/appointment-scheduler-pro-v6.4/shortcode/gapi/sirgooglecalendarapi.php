...
class GoogleCalendarApi
{
	 ...
	 function CreateCalendarEvent($calendar_id, $summary, $event_type, $event_date, $event_time, $event_timezone, $access_token) {
		...
		if($event_type == 1) {
			...
			$curlPost['start'] = array('dateTime' => $event_date.$event_time, 'timeZone' => $event_timezone);
	 ...
	 function UpdateCalendarEvent($event_id, $calendar_id, $summary, $all_day, $event_time, $event_timezone, $access_token) {
	 ...
	 function DeleteCalendarEvent($event_id, $calendar_id, $access_token) {
		$url_events = 'https://www.googleapis.com/calendar/v3/calendars/' . $calendar_id . '/events/' . $event_id;
    ...
    // $url = 'https://www.googleapis.com/calendar/v3/calendars/primary/events?showDeleted=false&$orderBy=email&maxResults='.$max_results.'&alt=json&v=3.0&oauth_token='.$token->access_token;
     ...
     function ReadEvents($access_token){
    	$url_events = "https://www.googleapis.com/calendar/v3/calendars/primary/events?maxResults='250'";
    	$ch = curl_init();    	
    	curl_setopt($ch, CURLOPT_URL, $url_events);	
    	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    	curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);

<?php
//ASSIGN VALUES IN CONFIRM TAB FIELDS ON LOGIN / REGISTER
global $wpdb;
...
function appoint_register(){
	...
	if(appointment_username==''){
	 ...
	 jQuery('#client_email').focus();
		...
		jQuery('#appoint_register').html('<i class="fas fa-spinner fa-spin"></i> <?php _e("Saving",WL_A_P_SYSTEM); ?>');
			jQuery.ajax({
			type : 'post',
            url : frontendajax.ajaxurl+'?action=login_ajax_request',  
            data :  'full_name='+ full_name   + '&client_password='+ appointment_client_password     + '&appoint_username='+ appointment_username + '&first_name='+ appointment_first_name +  '&last_name='+ appointment_last_name  +  '&client_contact='+ appointment_client_contact  +  '&client_email='+ appointment_client_email  +  '&skype_id='+ appointment_skype_id  +  '&appoint_notes='+ appointment_skype_id  +  '&appoint_notes='+ appointment_appoint_notes  + '&staff='+ staff_member + '&service='+ service + '&booking_st='+ booking_st  + '&apt_date='+ apt_date + '&apt_time_format='+ apt_time_format + '&ap_location_id='+ ap_location_id , 
...
function appoint_login(){
	...
	if(appointment_login_email==''){
	 ...
	 jQuery('#ap_password').focus();
		...
		jQuery('#appoint_login').html('<i class="fas fa-spinner fa-spin"></i> <?php _e("Loading",WL_A_P_SYSTEM); ?>');
		
		jQuery.ajax({
			type : 'post',
            url : frontendajax.ajaxurl+'?action=login_ajax_request',  
            data :  'login_email='+ appointment_login_email  + '&login_password='+ appointment_login_password  +  '&staff='+ staff_member + '&service='+ service + '&booking_st='+ booking_st + '&apt_date='+ apt_date + '&apt_time_format='+ apt_time_format + '&ap_location_id='+ ap_location_id, 
            success : function(data){
								jQuery("#appoint_login").prop('disabled', true);
								jQuery('#appoint_login').html('<i class="fas fa-spinner fa-spin"></i> <?php _e("Login",WL_A_P_SYSTEM); ?>');
...
<div class="col-md-12 col-sm-12 ap-steps">
	...
	<div id="3" class="ap-steps-detail3">
		<p><span><?php _e("You are about to request an appointment for",WL_A_P_SYSTEM); ?></span><span class="service_tag"><?php echo $apt_service_name; ?></span> <span>&nbsp;<?php _e("by",WL_A_P_SYSTEM); ?>&nbsp;</span><span class="staff_tag"><?php echo $apt_staff_name; ?>&nbsp;</span><span><?php _e("at",WL_A_P_SYSTEM); ?>&nbsp;</span><span class="time_tag"><?php echo $apt_time_format; ?>&nbsp;</span><span><?php _e("on",WL_A_P_SYSTEM); ?></span> 
		<?php 
		$date_format = get_option( 'date_format' );
		$appt_date_format=date($date_format, strtotime($apt_date));	?>
		<span class="date_tag"><?php echo $appt_date_format; ?></span>
		<span>.&nbsp;<?php _e("Price for the",WL_A_P_SYSTEM); ?>&nbsp;</span><span class="service_tag"><?php echo $apt_service_name; ?>&nbsp;</span><span><?php _e("is",WL_A_P_SYSTEM); ?>&nbsp;</span><span class="service_price_tag">&nbsp;<?php echo $apt_service_price; ?><?php if ($payment_currency==""){echo ' USD';} else{ echo ' '.$payment_currency;} ?></span>
		</p>
		...
		<!-- Step 3 -->
				...
				<form style="margin-bottom: 0;" action="" method="POST" id="ap_register" name="ap_register">
					...
					<div class="row">
						<div class="col-md-6 col-sm-6 step3-form">
							...
							<input type="text" class="form-control" name="appoint_username" id="appoint_username" placeholder="<?php _e("Enter Usename",WL_A_P_SYSTEM); ?>"/>
						</div>
						<div class="col-md-6 col-sm-6 step3-form">
						...
						<div class="col-md-6 col-sm-6 step3-form">
							...
							<input type="text" class="form-control" name="last_name" id="last_name" placeholder="<?php _e("Enter Last Name",WL_A_P_SYSTEM); ?>"/>
						</div>
						<div class="col-md-6 col-sm-6 step3-form">
			...
			<div id="existing" class="service-form" style="display:none;">
	...
	<div class="ap-step-link">
		...
		<button href="#step2" onclick="return step3_back();" class="btn step-left" type="button"  ><?php  if (!empty($details_navigation_text_backward)) { echo __("$details_navigation_text_backward",WL_A_P_SYSTEM); } else {  _e('Back',WL_A_P_SYSTEM); }  ?></button> 
		<button id="stepnext3" type="button" onclick="return step3_next();" class="btn step-right"><?php if (!empty($details_navigation_text_forward)) { echo __("$details_navigation_text_forward",WL_A_P_SYSTEM);  } else { _e('Next',WL_A_P_SYSTEM); }  ?></button> 
	</div>

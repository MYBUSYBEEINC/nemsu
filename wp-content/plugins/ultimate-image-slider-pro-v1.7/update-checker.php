...
if ( !class_exists('UISP_UpdateChecker') ):
...
class UISP_UpdateChecker {
	 ...
	 * @return void
	 */
	public function UISP_installHooks(){		
		//Add custom Plugin update menu at Dashboard update menu section.
		add_action('admin_menu', array( $this, 'UISP_my_Plugin_menu'));	
		//Add custom Plugin update available notice with link menu at top menu section.
		add_action('admin_bar_menu', array( $this, 'UISP_Plugin_update_link'), 99999999);
		//Insert our update css style by this function at footer.
		add_action('admin_footer', array( $this, 'UISP_my_admin_Plugin_update_function'));		
	}
	
	public function UISP_getNewVersion() {
		$details_url = 'https://weblizar.com/plugins/ultimate-image-slider-pro/';
		$download_url = 'https://weblizar.com/amember/login';
		$updation_detail = unserialize(get_option('wl-urisp-updation-detail'));
		...
		if( isset( $updation_detail['next_check'] ) && ( strtotime(date('Y-m-d')) >= strtotime($updation_detail['next_check']) ) ) {
			$update_version = $this->UISP_requestUpdate();
			if($update_version) {
				$new_version->version = $update_version->version;
			} else {
				$new_version->version = $this->UISP_getInstalledVersion();
			}
	...
	public function UISP_Plugin_update_link($wp_admin_bar) 
	{
		$new_version = $this->UISP_getNewVersion();		
		$old_version = $this->UISP_getInstalledVersion();
		if ( is_object($new_version) && version_compare($new_version->version, $old_version, '>') )
		{
		$args = array(
				'id' => 'UISP_Plugin_update_menu',
				'title' => 'Ultimate Image Slider Pro <span style="color:#fff;font-weight:bold;">Update Available !</span>', 
				'href' => get_admin_url().'index.php?page=UISP_new_updates',  
				'meta' => array(
					'class' => 'UISP_update-link',
					'title' => 'UISP_Update Available'
					)
			);
	...
	public function UISP_my_Plugin_menu() {
		$new_version = $this->UISP_getNewVersion();		
		$old_version = $this->UISP_getInstalledVersion();
		if ( is_object($new_version) && version_compare($new_version->version, $old_version, '>') )
		{
			add_dashboard_page('Ultimate Image Slider Pro Update', 'Ultimate Image Slider Pro Update', 'read', 'UISP_new_updates', array( $this, 'UISP_my_Plugin_function'));
		}
	}
	public function UISP_my_Plugin_function() { 
		$new_version = $this->UISP_getNewVersion();		
		$old_version = $this->UISP_getInstalledVersion();
		if ( is_object($new_version) && version_compare($new_version->version, $old_version, '>') )
		{
		...
		$msg = sprintf( $msg, $this->product_name );
			...
			<div class="weblizar_new_updates">
				...
				<a target="_blank" href="<?php echo UISP_WEBLIZAR_PLUGIN_URL; ?>" title="Installing Weblizar WordPress Plugin"><?php _e($this->product_name);?></a>
				</h1>
				<div class="weblizar_new_updates_details">
					...
					<h2 class="weblizar_new_updates_details_main_headings" ><?php _e('Weblizar Plugin Update Process','urisp');?></h2>			
						...
						<p><img style="margin: 20px 0; width: 750px;" src="<?php echo UISP_PLUGIN_URL .'assets/images/institute-pro.png'; ?>" alt="Ultimate Image Slider Pro"></p>
						...
						<p><img style="margin: 20px 0; width: 750px;" src="<?php echo UISP_PLUGIN_URL .'assets/images/install-institute-pro.png'; ?>" alt="Install Ultimate Image Slider Pro"></p>
						<p><?php _e('Your Weblizar pro Plugin Activated Now','urisp');?></p>
						<p><?php _e('For more details to follow the Plugin documentation link : <a target="_blank" href='.UISP_WEBLIZAR_PLUGIN_URL.'>'.UISP_WEBLIZAR_PLUGIN_URL.'</a>','urisp');?></p>
	...
	<?php
		...
		function UISP_my_admin_Plugin_update_function() { ?>	
			<style>
				...
				#wp-admin-bar-UISP_Plugin_update_menu.UISP_update-link{background-color:#31A3DD !Important;border-radius:5px!important;}
				#wp-admin-bar-UISP_Plugin_update_menu.UISP_update-link a{color:#fff !Important;}
				#wp-admin-bar-UISP_Plugin_update_menu{margin-right:5px !Important;}				
 ...
 /**
	 ...
	 * @return PluginUpdate 
	 */
	public function UISP_requestUpdate($queryArgs = array()){
		//Query args to append to the URL. Plugins can add their own by using a filter callback (see addQueryArgFilter()).
		$queryArgs['installed_version'] = $this->UISP_getInstalledVersion(); 
		...
		if ( ($code == 200) && !empty($body) ){
			...
			if ($PluginUpdate != null) {
				$updationDetail = serialize(array(
							'current_version' => $this->UISP_getInstalledVersion(),
							'new_version' => $PluginUpdate->version,
							'checked_on' => date('Y-m-d'),
							'check_again' => version_compare($PluginUpdate->version, $this->UISP_getInstalledVersion(), '>') ? true : false, 
							'next_check' => date('Y-m-d', strtotime("+1 day"))
						));
				update_option('wl-urisp-updation-detail', $updationDetail);
			}

			//The update should be newer than the currently installed version.
			if ( ($PluginUpdate != null) && version_compare($PluginUpdate->version, $this->UISP_getInstalledVersion(), '<') ){
				$PluginUpdate = NULL;
			}
		}
	 ...
	 */
	public function UISP_getInstalledVersion(){
		return UISP_VERSION;
	}
}
...
$weblizar_update_checker = new UISP_UpdateChecker('Ultimate Image Slider Pro','https://weblizar.com/updates/plugins/ultimate-image-slider-pro.json');
...
require_once RRPPP_PLUGIN_DIR_PATH . '/admin/WL_RRPPP_LC.php';

final class WL_RRPPP_LM {
    ...
    private function __construct() {
        $this->license_key = trim( get_option( 'wl-rrppp-key' ) );
    }
    ...
    public function is_valid() {
        ...
        if( get_option( 'wl-rrppp-valid' ) ) {
            return true;
        }
        if( $this->key_exists() ) {
            $checker = new WL_RRPPP_LC( $this->license_key, $this->api_url, md5( $this->license_key ) );
            if ( ! $checker->checkLicenseKey() ) {
                $this->error_message = $checker->getMessage();
                return false;
            } else {
                $this->error_message = null;
                $activation_cache = trim( get_option( 'wl-rrppp-cache' ) );
                ...
                update_option( 'wl-rrppp-key', $this->license_key );
                update_option( 'wl-rrppp-valid', true );
                if ( $prev_activation_cache != $activation_cache ) {
                    update_option( 'wl-rrppp-cache', $activation_cache );
                }
                return true;
            }
        }

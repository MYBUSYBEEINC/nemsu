 ...
 *    $checker = new WL_RRPPP_LicenseChecker($_GET['license_key']);
 ...
 *     $checker = new WL_RRPPP_LicenseChecker(file_get_contents('license_key.php'));
...
class WL_RRPPP_LC {
    ...
    private $_local_encryption_key = 'b3d822f5f1c9bd243cbe0d51684b036c858c50b1';
    ...
    /** @var WL_RRPPP_LicenseChecker_ActivationResponse */
    ...
    function makeRequest($method, $action, $params, $errorCode, $responseClass = 'stdclass') {
        ...
        list($body, $status, $error) = $this->openUrl(
            $method,
            $this->url . '/' . $action,
            $params
        );
    ...
    protected function processActivationResponse($resp) {
        ...
        switch ($response->code) {
            ...
            case self::LICENSE_EXPIRED:
                if (
                    (string)$response->grace_period == 'true' ||
    ...
    private function decodeResponse($response) {
        $ret = new WL_RRPPP_LicenseChecker_ActivationResponse;
        if (empty($response))
            return $ret;
        $c = new WL_RRPPP_Crypt_Blowfish($this->_local_encryption_key);
        try {
    ...
    public function getRequest() {
        ...
        foreach ($this->request_vars as $k)
            switch ($k) {
                case 'ip':
                    $ret[$k] = $this->getServerIp();
                    break;
                case 'domain':
                    $ret[$k] = $this->getDomain();
                    break;
                case 'sdomain':
                    $ret[$k] = $this->getSdomain();
                    break;
                case 'url':
                    $ret[$k] = $this->getRootUrl();
                    break;
                case 'hardware-id':
                    $ret[$k] = $this->getHardwareId();
                    break;
            }
        ksort($ret);
        $this->_request = $ret;
...
class WL_RRPPP_LicenseChecker_ActivationResponse {
    public $code;
    public $message;
...
class WL_RRPPP_Crypt_Blowfish {
    ...
    function _init() {
        $defaults = new WL_RRPPP_Crypt_Blowfish_DefaultKey();
        $this->_P = $defaults->P;
        $this->_S = $defaults->S;
    }
...
class WL_RRPPP_Crypt_Blowfish_DefaultKey {
    var $P = array();

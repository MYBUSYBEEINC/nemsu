...
if ( !class_exists('RPRP_UpdateChecker') ):
...
class RPRP_UpdateChecker {
	 ...
	 * @return void
	 */
	public function RPRP_installHooks(){		
		//Add custom Plugin update menu at Dashboard update menu section.
		add_action('admin_menu', array( $this, 'RPRP_my_Plugin_menu'));	
		//Add custom Plugin update available notice with link menu at top menu section.
		add_action('admin_bar_menu', array( $this, 'RPRP_Plugin_update_link'), 99999999);
		//Insert our update css style by this function at footer.
		add_action('admin_footer', array( $this, 'RPRP_my_admin_Plugin_update_function'));		
	}

		public function RPRP_getNewVersion() {
		$details_url = 'https://weblizar.com/plugins/recent-related-post-and-page-pro';
		$download_url = 'https://weblizar.com/amember/login';
		$updation_detail = unserialize(get_option('RPRP_updation_detail'));
		...
		if( isset( $updation_detail['next_check'] ) && ( strtotime(date('Y-m-d')) >= strtotime($updation_detail['next_check']) ) ) {
			$update_version = $this->RPRP_requestUpdate();
			if($update_version) {
				$new_version->version = $update_version->version;
			} else {
				$new_version->version = $this->RPRP_getInstalledVersion();
			}
	...
	public function RPRP_Plugin_update_link($wp_admin_bar) 
	{
		$new_version = $this->RPRP_getNewVersion();		
		$old_version = $this->RPRP_getInstalledVersion();
		if ( is_object($new_version) && version_compare($new_version->version, $old_version, '>') )
		{
		$args = array(
				'id' => 'RPRP_Plugin_update_menu',
				'title' => 'Recent Related Post and Page Pro <span style="color:#fff;font-weight:bold;">Update Available !</span>', 
				'href' => get_admin_url().'index.php?page=RPRP_new_updates',  
				'meta' => array(
					'class' => 'RPRP_update-link',
					'title' => 'RPRP_Update Available'
					)
			);
	...
	public function RPRP_my_Plugin_menu() {
		$new_version = $this->RPRP_getNewVersion();		
		$old_version = $this->RPRP_getInstalledVersion();
		if ( is_object($new_version) && version_compare($new_version->version, $old_version, '>') )
		{
			add_dashboard_page('Recent Related Post and Page Pro Update', 'Recent Related Post and Page Pro Update', 'read', 'RPRP_new_updates', array( $this, 'RPRP_my_Plugin_function'));
		}
		
	}
	public function RPRP_my_Plugin_function(){ 
		$new_version = $this->RPRP_getNewVersion();		
		$old_version = $this->RPRP_getInstalledVersion();
		if ( is_object($new_version) && version_compare($new_version->version, $old_version, '>') )
		{
		...
		$msg = sprintf( $msg, $this->product_name );
			...
			<div class="weblizar_new_updates">
				...
				<a target="_blank" href="<?php echo RPRP_PLUGIN_LINK; ?>" title="Installing Weblizar WordPress Plugin"><?php _e($this->product_name);?></a>
				</h1>
				<div class="weblizar_new_updates_details">
					...
					<h2 class="weblizar_new_updates_details_main_headings" ><?php _e('Weblizar Plugin Update Process','weblizar');?></h2>			
						...
						<p><img style="margin: 20px 0; width: 750px;" src="<?php echo plugin_dir_url( __FILE__ ).'settings/images/recent-post-pro.jpg'; ?>" alt="wp-dashboard"></p>
						...
						<p><img style="margin: 20px 0; width: 750px;" src="<?php echo plugin_dir_url( __FILE__ ).'settings/images/install-recent.jpg'; ?>" alt="wordpress-Plugin-install"></p>
						<p><?php _e('Your Weblizar pro Plugin Activated Now','weblizar');?></p>
						<p><?php _e('For more details to follow the Plugin documentation link ','weblizar');?><a target="_blank" href='https://weblizar.com/documentation/recent-related-post-and-page-pro/'><?php _e("Recent Related Post and Page Pro",'weblizar'); ?></a></p>
	...
	<?php
		...
		function RPRP_my_admin_Plugin_update_function() { ?>	
			<style>
				...
				#wp-admin-bar-RPRP_Plugin_update_menu.RPRP_update-link{background-color:#31A3DD !Important;border-radius:5px!important;}
				#wp-admin-bar-RPRP_Plugin_update_menu.RPRP_update-link a{color:#fff !Important;}
				#wp-admin-bar-RPRP_Plugin_update_menu{margin-right:5px !Important;}				
 ...
 /**
	 ...
	 * @return PluginUpdate 
	 */
	public function RPRP_requestUpdate($queryArgs = array()){
		//Query args to append to the URL. Plugins can add their own by using a filter callback (see addQueryArgFilter()).
		$queryArgs['installed_version'] = $this->RPRP_getInstalledVersion(); 
		...
		if ( ($code == 200) && !empty($body) ){
			...
			if ($PluginUpdate != null) {
				$updationDetail = serialize(array(
							'current_version' => $this->RPRP_getInstalledVersion(),
							'new_version' => $PluginUpdate->version,
							'checked_on' => date('Y-m-d'),
							'check_again' => version_compare($PluginUpdate->version, $this->RPRP_getInstalledVersion(), '>') ? true : false, 
							'next_check' => date('Y-m-d', strtotime("+1 day"))
						));
				update_option('RRPPP_updation_detail', $updationDetail);
			}
			
			//The update should be newer than the currently installed version.
			if ( ($PluginUpdate != null) && version_compare($PluginUpdate->version, $this->RPRP_getInstalledVersion(), '<') ){
				$PluginUpdate = null;
			}
	 ...
	 */
	public function RPRP_getInstalledVersion(){
		return RPRP_VERSION;
	}
}
...
$weblizar_update_checker = new RPRP_UpdateChecker('Recent Related Post and Page Pro','https://weblizar.com/updates/plugins/recent-related-post-and-page-pro.json');
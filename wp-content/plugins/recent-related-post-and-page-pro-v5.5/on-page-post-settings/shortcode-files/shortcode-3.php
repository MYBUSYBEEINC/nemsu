...
<style>
	...
	#r_r_post_date_<?php echo esc_attr($p_o_s_t)?>
	{
		...
		top:<?php echo esc_attr($date_font_size)?>px !important;
		<?php if($featured_image_3=="none") { echo 'position: initial !important;top:0px !important;';}?>
	...
	#r_r_post_image_<?php echo esc_attr($p_o_s_t)?>
	{
		...
		height: <?php echo esc_attr($back_image_height_3) ?>px !important;
		width:100% !important;
		<?php if($featured_image_3=="none") { echo 'height: 0px !important;margin-bottom: 70px !important;';}?>
	}

	#r_r_post_title_<?php echo esc_attr($p_o_s_t)?>
	{
		margin-bottom: 10px !important;
		<?php if($featured_image_3=="none"){ echo ' margin-bottom: 30px !important;';}?>
	...
	#r_r_post_content_<?php echo esc_attr($p_o_s_t)?>.web_ticker_content
	{
		...
		font-family: <?php echo esc_attr($NWT_Font_Style) ?> !important;
		word-wrap:break-word !important;
		<?php if($featured_image_3=="none") { echo ' margin-top: 40px;';}?> !important;
	}
...
<?php if(is_singular('page')){?> <h1 id="page_post_info_<?php echo esc_attr($p_o_s_t).$a; ?>"  class="weblizar_page_post_info" align="center"><?php echo esc_attr($p_p_lbl_text_page); ?></h1> <?php } ?>
	...
	<div  id="outer_div_<?php echo esc_attr($p_o_s_t); ?>" style="width:100%" >
		...
		if ( $the_query->have_posts() )
		{
			...
			while ( $the_query->have_posts() )
			{
				...
				<?php $date_post=get_the_date(); ?>
				<?php  if($featured_image_3=="default"){ $my_image=wp_get_attachment_url( get_post_thumbnail_id($post_info)); }   if($featured_image_3=="upload"){$my_image=$back_user_image_3; }  if($featured_image_3=="none"){$my_image=""; } ?>
				...
				<div id="r_r_post_image_<?php echo esc_attr($p_o_s_t)?>" style=" background-image:url('<?php echo esc_url($my_image); ?>') !important; <?php if($my_image=='') { echo 'height:30px !important';}?>" >
<?php if($show_hide=="show"){?>	<div id="r_r_post_date_<?php echo esc_attr($p_o_s_t)?>" align="<?php echo esc_attr($date_align) ?>"><span id="date_span_date_<?php echo $p_o_s_t?>" class="date1"> <?php  echo esc_attr($date_post) ?></span></div> <?php } ?>
				</div>
				<div id="r_r_post_content_<?php echo esc_attr($p_o_s_t)?>" class="web_ticker_content"style=""><?php  echo esc_attr($content)?></div>
				<div align="<?php echo esc_attr($link_align) ?>" id="r_r_post_link_<?php echo esc_attr($p_o_s_t)?>" > <a href="<?php the_permalink(); ?>" class="r_r_p_link"> <?php echo esc_attr($link_text) ?></a></div>

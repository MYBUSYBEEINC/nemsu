...
<style>
	...
	#r_r_post_date_<?php echo $p_o_s_t?>
	{
		...
		top: 0px !important;
		padding: 10px !important;
<?php if($show_hide=="show"){?>	 background-color: <?php echo esc_attr($date_back_Color); ?> !important; <?php } ?>
	...
	<div  id="outer_div_<?php echo esc_attr($p_o_s_t); ?>" style="width:100%" >
		...
		if ( $the_query->have_posts() )
		{
			...
			while ( $the_query->have_posts() )
			{
				...
				<?php  if($featured_image_6=="default"){ $my_image=wp_get_attachment_url( get_post_thumbnail_id($post_info)); }   if($featured_image_6=="upload"){$my_image=$back_user_image_6; }  if($featured_image_6=="none"){$my_image=""; } ?>

				<div id="r_r_post_image_<?php echo esc_attr($p_o_s_t);?>" style=" background-image:url('<?php echo esc_url($my_image); ?>') !important;" >

					<div id="r_r_post_date_<?php echo esc_attr($p_o_s_t)?>"> <?php if($show_hide=="show"){?> <span id="date_span_date_<?php echo esc_attr($p_o_s_t);?>" class="date1"> <?php  echo esc_attr($date_post); ?></span> <?php } ?></div>
					<div id="r_r_post_content_<?php echo esc_attr($p_o_s_t);?>" class="web_ticker_content"style=""><?php  echo esc_attr($content);?></div>
					<div align="" id="r_r_post_link_<?php echo esc_attr($p_o_s_t);?>" ><a href="<?php the_permalink(); ?>" class="r_r_p_link"> <?php echo esc_attr($link_text); ?></a></div>

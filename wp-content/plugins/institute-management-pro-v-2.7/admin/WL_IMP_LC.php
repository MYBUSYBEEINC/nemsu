 ...
 *    $checker = new WL_IMP_LicenseChecker($_GET['license_key']);
 ...
 *     $checker = new WL_IMP_LicenseChecker(file_get_contents('license_key.php'));
...
class WL_IMP_LC 
{
    ...
    private $_local_encryption_key = '992299870911f888e796165b7ca128924c637336';
    ...
    /** @var WL_IMP_LicenseChecker_ActivationResponse */
     ...
     */
    public function __construct($key, $url, $hash) 
    {
        $this->key = $key;
        $this->url = $url;
        $this->hash = $hash;
    ...
    function makeRequest($method, $action, $params, $errorCode, $responseClass = 'stdclass')
    {
        ...
        list($body,$status,$error) = $this->openUrl($method, $this->url . '/' . $action,
            $params);
        if ($status != 200)
        {
     ...
     * @return \WL_IMP_LicenseChecker_ActivationResponse
     */
    protected function processActivationResponse($resp)
    {
        $response = new WL_IMP_LicenseChecker_ActivationResponse($resp);
    ...
    {
        $ret = new WL_IMP_LicenseChecker_ActivationResponse;
        if (empty($response)) 
            return $ret;
        $c = new WL_IMP_Crypt_Blowfish($this->_local_encryption_key);
        try {
    ...
    {
        if (!is_object($response)) return null;
    ...
    public function getRequest()
    {
        ...
        foreach ($this->request_vars as $k)
            switch ($k)
            {
                case 'ip' : $ret[$k] = $this->getServerIp(); break;
                case 'domain' : $ret[$k] = $this->getDomain(); break;
                case 'sdomain' : $ret[$k] = $this->getSdomain(); break;
                case 'url' : $ret[$k] = $this->getRootUrl(); break;
                case 'hardware-id': $ret[$k] = $this->getHardwareId(); break;
            }
        ksort($ret);
        $this->_request = $ret;
        return $ret;
...
class WL_IMP_LicenseChecker_ActivationResponse
{
    public $code;
    public $message;
...
class WL_IMP_Crypt_Blowfish
{
    ...
    function _init()
    {
        $defaults = new WL_IMP_Crypt_Blowfish_DefaultKey();
        $this->_P = $defaults->P;
        $this->_S = $defaults->S;
    }
...
class WL_IMP_Crypt_Blowfish_DefaultKey
{
    var $P = array();

...
class WLSM_M {
	...
	public static function study_materials_query() {
		return 'SELECT sm.ID, sm.label as title, sm.description, sm.attachments, sm.created_at FROM ' . WLSM_CLASS_SCHOOL_STUDY_MATERIAL . ' as cssm 
	...
	public static function study_material_query() {
		return 'SELECT sm.ID, sm.label as title, sm.description, sm.url, sm.attachments, sm.created_at FROM ' . WLSM_CLASS_SCHOOL_STUDY_MATERIAL . ' as cssm JOIN ' . WLSM_STUDY_MATERIALS . ' as sm ON sm.ID = cssm.study_material_id WHERE cssm.class_school_id = %d AND sm.ID = %d';
	}
	...
	public static function meetings_query() {
		return 'SELECT mt.ID, mt.host_id, mt.meeting_id, mt.recordable, mt.topic, mt.duration, mt.start_at, mt.type, mt.password, mt.join_url, sj.label as subject_name, se.ID as section_id, sj.code as subject_code, a.name as name FROM ' . WLSM_MEETINGS . ' as mt

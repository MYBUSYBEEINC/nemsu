...
class WLSM_M_Staff_General {
	...
	public static function get_student_attendance_report( $student_id ) {
		...
		$attendance = $wpdb->get_results(
			$wpdb->prepare( 'SELECT Year(attendance_date) as year, Month(attendance_date) as month, COUNT(student_record_id) as total_attendance, COALESCE(SUM(if(status = "a", 1, 0)), 0) as total_absent, COALESCE(SUM(if(status = "p", 1, 0)), 0) as total_present, attendance_date FROM ' . WLSM_ATTENDANCE . ' WHERE student_record_id = %d AND status IN("a", "p") GROUP BY Year(attendance_date), Month(attendance_date) ORDER BY year DESC, month DESC', $student_id )
		);
		return $attendance;
	}

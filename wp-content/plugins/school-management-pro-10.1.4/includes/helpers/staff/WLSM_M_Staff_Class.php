...
class WLSM_M_Staff_Class
{
	...
	public static function get_class_students($school_id, $session_id, $class_id, $only_active = true)
	{
		...
		$students = $wpdb->get_results($wpdb->prepare('SELECT sr.ID, sr.name, sr.enrollment_number, sr.roll_number, sr.phone, sr.father_name, sr.father_phone, se.label as section_label FROM ' . WLSM_STUDENT_RECORDS . ' as sr
			...
			JOIN ' . WLSM_CLASS_SCHOOL . ' as cs ON cs.ID = se.class_school_id
			WHERE cs.school_id = %d AND ss.ID = %d AND cs.class_id = %d' . $where . ' GROUP BY sr.ID ORDER BY LENGTH(sr.roll_number), sr.roll_number ASC, sr.name ASC', $school_id, $session_id, $class_id), OBJECT_K);
		return $students;
	}
	...
	{
		global $wpdb;
		$notice = $wpdb->get_row($wpdb->prepare('SELECT n.ID, n.title, n.attachment, n.url, n.link_to, n.is_active FROM ' . WLSM_NOTICES . ' as n 

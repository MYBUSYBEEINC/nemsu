	...
	<div class="wlsm-st-leave-request-section">
		<form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="post" id="wlsm-submit-student-homework-submission-form">
			...
			<div class="row">
				...
				<div class="wlsm-col-6 wlsm-form-group">
					...
					<div class="embed-responsive embed-responsive-16by9">
					  <iframe class="embed-responsive-item" src="<?php echo esc_url( wp_get_attachment_url( $homework->attachments ) ); ?>" ></iframe>
					</div>
					<?php } }?>
			...
			<div class="wlsm-form-group">
				...
				<textarea required name="description" class="wlsm-font-control" id="wlsm_description" placeholder="<?php esc_attr_e('Enter here', 'school-management'); ?>" cols="30" rows="4" > <?php if ( $homework->description && isset($id)) { echo "$homework->description"; } ?></textarea>

				<?php } } else{ ?>
					<textarea required name="description" class="wlsm-font-control" id="wlsm_description" placeholder="<?php esc_attr_e('Enter here', 'school-management'); ?>" cols="30" rows="4" > 
					<?php if ( $homework && isset($id)) {
							 echo "$homework->description"; 
							} ?>
					</textarea>
					<?php } ?>
			</div>
			<div class="wlsm-border-top wlsm-pt-2 wlsm-mt-1">
				<button data-confirm="<?php esc_attr_e('Confirm! Are you sure?', 'school-management'); ?>" class="button wlsm-btn btn btn-primary" type="submit" id="wlsm-submit-student-homework-submission-btn">

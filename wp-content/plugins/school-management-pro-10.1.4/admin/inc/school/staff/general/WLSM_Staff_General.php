...
class WLSM_Staff_General
{
	public static function get_class_sections() {
		...
		try {
			...
			$sections = array_map(function ($sections, $subject, $fees) {
				if (!empty($sections->label)) {
					$sections->label = WLSM_M_Staff_Class::get_section_label_text($sections->label);
				}
				return ['section'=> $sections, 'subject'=> $subject, 'fees' => $fees];
				...
				if ($student_id) {
					...
					if ($subjects_ids){
						foreach ($subjects_ids as $subject_id) {
							...
							$subject_id = $wpdb->insert(WLSM_STUDENTS_SUBJECTS, $subject_data);
							if (is_wp_error($subject_id)) {
								throw new Exception($subject_id->get_error_message());
							}
						}
					}

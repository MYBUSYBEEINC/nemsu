...
class WLSM_Staff_Accountant {
				...
				if (count($filter_rows_limit)) {
					foreach ($filter_rows_limit as $row) {
						...
						$data[] = array(
							...
							'<a class="text-success wlsm-print-invoice" data-nonce="' . esc_attr(wp_create_nonce('print-invoice-' . $row->ID)) . '" data-invoice="' . esc_attr($row->ID) . '" href="#" data-message-title="' . esc_attr__('Print Invoice', 'school-management') . '" data-close="' . esc_attr__('Close', 'school-management') . '"><i class="fas fa-print"></i></a>&nbsp;&nbsp;
							<a class="text-primary" href="' . esc_url($page_url . "&action=save&id=" . $row->ID) . '"><span class="dashicons dashicons-edit"></span></a>' . ($can_delete_invoices ? ('&nbsp;&nbsp;
	...
	public static function print_bulk_invoices() {
		...
		$class_id   = isset($_POST['class_id']) ? absint($_POST['class_id']) : 0;
		$section_id = isset($_POST['section_id']) ? absint($_POST['section_id']) : 0;
		$is_paid    = isset($_POST['paid']) ? (bool) ($_POST['paid']) : 1;
	...
	public static function fetch_expenses() {
		...
		// Total query.
		$rows_query = WLSM_M_Staff_Accountant::fetch_expense_query_count($school_id, $session_id);

...
class WLSM_Staff_Class
{
	public static function fetch_classes()
	{
		...
		if (count($filter_rows_limit)) {
			foreach ($filter_rows_limit as $row) {
				...
				$add_section = '<a class="btn btn-primary btn-sm" href="' . esc_url($page_url . "&action=sections&id=" . $row->ID) . '">' . esc_html__('Manage', 'school-management') . '</a>';
				if (!$sections_count) {
					$sections_count = 0;
				} else {
				...
				$data[] = array(
					...
					$sections_count . '<span class="ml-4">' . $add_section . '<span>',
	...
	public static function fetch_event_participants()
	{
		...
		// Ordering.
		$columns = array('sr.name', 'sr.enrollment_number', 'c.label', 'se.label', 'sr.roll_number', 'sr.phone');
		if (isset($_POST['order']) && isset($columns[$_POST['order']['0']['column']])) {
			$order_by  = sanitize_text_field($columns[$_POST['order']['0']['column']]);
			$order_dir = sanitize_text_field($_POST['order']['0']['dir']);
	...
	public static function save_meeting() {
		...
		try {
			...
			if ($meeting_id) {
				$zoom_meeting_id = trim($meeting->meeting_id);

				if (!$zoom_meeting_id) {
					wp_send_json_error(esc_html__('Invalid Zoom meeting ID.', 'school-management'));
				}
				$response = $meetings->update($zoom_meeting_id, $api_data);

				if (!isset($response['code'])) {
					$api_error = esc_html__('Unable to update meeting on Zoom.', 'school-management');
				} elseif (204 !== $response['code']) {
					$api_error = isset($response['message']) ? $response['message'] : esc_html__('Unable to update meeting on Zoom.', 'school-management');
				}
			} else {
				$response = $meetings->create(WLSM_Helper::meeting_host(), $api_data);

				if (!isset($response['code'])) {
					$api_error = esc_html__('Unable to create meeting on Zoom.', 'school-management');
				} elseif (201 !== $response['code']) {
					$api_error = isset($response['message']) ? $response['message'] : esc_html__('Unable to create meeting on Zoom.', 'school-management');
				}
			}

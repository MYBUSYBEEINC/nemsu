...
class WLSM_Database
{
	public static function activation()
	{
		...
		$row = $wpdb->get_results("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '" . DB_NAME . "' AND TABLE_NAME = '" . WLSM_PAYMENTS . "' AND COLUMN_NAME = 'added_by'");
		if (empty($row)) {
		...
		$sql = "CREATE TABLE IF NOT EXISTS " . WLSM_STUDENTS_SUBJECTS . " (
			...
			subject_id bigint(20)  DEFAULT NULL UNIQUE,
			PRIMARY KEY (ID)
			) ENGINE=InnoDB " . $charset_collate;
		dbDelta($sql);

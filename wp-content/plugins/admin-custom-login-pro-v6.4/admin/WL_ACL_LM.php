...
require_once WEBLIZAR_ACL_PLUGIN_DIR_PATH_PRO . '/admin/WL_ACL_LC.php';

final class WL_ACL_LM {
	...
	private function __construct() {
		$this->license_key = trim( get_option( 'wl-acl-key' ) );
	}
	...
	public function is_valid() {
		...
		if ( get_option( 'wl-acl-valid' ) ) {
			return true;
		}
		if ( $this->key_exists() ) {
			$checker = new WL_ACL_LC( $this->license_key, $this->api_url, md5( $this->license_key ) );
			if ( ! $checker->checkLicenseKey() ) {
				...
				$activation_cache      = trim( get_option( 'wl-acl-cache' ) );
				...
				update_option( 'wl-acl-key', $this->license_key );
				update_option( 'wl-acl-valid', true );
				if ( $prev_activation_cache != $activation_cache ) {
					update_option( 'wl-acl-cache', $activation_cache );
				}
